...
def detect_ring_centerline(gray, point, normal, search_length, template):
    h, w = gray.shape
    x0, y0 = point
    nx, ny = normal

    distances = np.arange(-search_length, search_length + 1, dtype=np.float64)
    ...
    vx = sample_x[valid]
    vy = sample_y[valid]
    values = gray[vy.astype(np.int32), vx.astype(np.int32)].astype(np.float64)

    if len(values) < len(template) + 4:
        return None

    response = np.convolve(values, template, mode="valid")

    pos_idx = int(np.argmax(response))
    neg_idx = int(np.argmin(response))

    if response[pos_idx] < 15.0 or response[neg_idx] > -15.0:
        return None

    return float(vx[pos_idx]), float(vy[pos_idx])
